...
@interface AVAsset (SWIFT_EXTENSION(Filterchain))
- (UIInterfaceOrientation)videoOrientation SWIFT_WARN_UNUSED_RESULT;
@end

@class RenderView;
@class UIImagePickerController;

SWIFT_CLASS("_TtC11Filterchain11FilterChain")
@interface FilterChain : NSObject <UIImagePickerControllerDelegate, UINavigationControllerDelegate>
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
/// Initialize all filter operations.
- (void)initFilters SWIFT_METHOD_FAMILY(none);
/// Start the filter chain
- (void)start;
/// Pass the view from the ViewController
- (void)startCameraWithViewWithView:(RenderView * _Nonnull)view;
/// Picks several random filters in the FilterOperations array and adds it to ActiveFilters
- (void)randomizeFilterChain;
/// Switches to the next filter available in filterOperations and appends it to ActiveFilters
- (void)nextFilterAvailable;
...
@interface UIImage (SWIFT_EXTENSION(Filterchain))
